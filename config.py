...
HOTKEY_CONFIGS = {
    ...
    'ctrl+shift+2': {
        'name': "文字识别与翻译",
        'prompt': f"请提取并返回这张图片选定区域中的所有文字。并且这些文字翻译为**{TARGET_LANGUAGE}**。如果图中是漫画，则按照漫画的阅读顺序进行转录和翻译。在回复中，首先提供原文，将全部原文使用一对圆括号()括起来。然后提供{TARGET_LANGUAGE}翻译，将整个翻译结果使用一对**英文方括号[]**，即[]符号，将答案括起来。例如[答案]。（即使有多段，也全放进一对英文方括号[]中）。",
